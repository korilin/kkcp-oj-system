...
function sendCode() {
  startCounter();
  Apis.LoginModule.sendCode(loginState.email).then((body) => {
    if (body.status) {
...
function tryLogin() {
  ...
  Apis.LoginModule.doLogin(data).then((body) => {
    if (body.status) {
    ...
    <div style="text-align: left;">
      ...
      <a-form :model="loginState" class="form login-form-global-style" @finish="tryLogin">
        ...
        <a-form-item class="vc-form-item">
          <a-input v-model:value="loginState.code" type="text" placeholder="Verification Code">
            <template #prefix>
