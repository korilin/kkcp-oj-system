...
const descriptionRole = async (_rule, value) => {
...
const codeTemplateRole = async (_rule, value) => {
...
const testDataJsonRole = async (_rule, value) => {
...
<template>
  ...
  >
    <a-form-item>
      <a-alert message="请先仔细阅读平台文档" type="info" show-icon style="margin:20px 0px 20px 150px;" />
    </a-form-item>
    ...
    <a-form-item label="Type" has-feedback name="type">
      <a-radio-group v-model:value="formState.type">
        <a-radio v-for="qType in commonStore.questionTypes" :value="qType.id">{{ qType.text }}</a-radio>
      </a-radio-group>
    </a-form-item>
    <a-form-item label="Level" has-feedback name="level">
      <a-radio-group v-model:value="formState.level">
        <a-radio v-for="level in commonStore.questionLevels" :value="level.id">{{ level.text }}</a-radio>
      </a-radio-group>
    </a-form-item>
